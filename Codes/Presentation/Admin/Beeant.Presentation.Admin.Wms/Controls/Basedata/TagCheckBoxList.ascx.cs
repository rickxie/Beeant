...
{
    public partial class TagCheckBoxList : DropDownListTemplateBaseControl<TagGroupEntity>
    {
        ...
        public virtual void LoadTags(string value)
        {
            ...
            if (infos != null)
            {
                ...
                {
                    Inputs.Append(string.Format("<input type='checkbox' name='{0}ckgSelected' value='{3}' {1}><span>{2}</span>", ClientID,
        ...
        protected void LoadTags()
        {
            ...
            {
                foreach (var info in infos)
                    Inputs.Append(string.Format("<input type='checkbox' name='{0}ckgSelected' value='{3}' {1}><span>{2}</span>",
        ...
        protected virtual void CreateClientScript()
        {
            ...
            var clientAjax = string.Format("$.ajax(" + Environment.NewLine +
                                           ...
                                           "                {{" + Environment.NewLine +
                                           "                    $(\"#{0}ckg\").append('" +
                                           string.Format("<input name=\"{0}ckgSelected\" type=\"checkbox\" value=' + this.Value + '>' + this.Text);", ClientID) + Environment.NewLine +
                                           "                }});" + Environment.NewLine +
                                           "            }}" + Environment.NewLine +
                                           "        }}," + Environment.NewLine +
