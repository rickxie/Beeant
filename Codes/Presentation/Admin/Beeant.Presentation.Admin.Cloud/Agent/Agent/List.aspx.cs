...
{
    public partial class List : ListPageBase<AgentEntity>
    {

        public virtual string GetBindName(AccountEntity account)
        {
            if (account == null || account.AccountNumbers == null || account.AccountNumbers.FirstOrDefault(it => it.Name == "AgentId") == null)
                return "未绑定";
            return "已绑定";
        }
        ...
        /// 得到价格实体
        /// </summary>
        protected virtual IList<AgentEntity> GetAgents()
        {
            ...
            query.Query<AgentEntity>().Where(it => ids.ToArray().Contains(it.Id)).Select(it => new object[] { it.Id, it.Account.Id });
            return Ioc.Resolve<IApplicationService, AgentEntity>().GetEntities<AgentEntity>(query);
        }
    }
}