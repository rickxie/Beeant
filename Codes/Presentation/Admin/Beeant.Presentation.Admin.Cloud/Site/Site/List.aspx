<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="List.aspx.cs" Inherits="Beeant.Presentation.Admin.Cloud.Site.Site.List" MasterPageFile="~/Main.Master" %>
<%@ Import Namespace="Component.Extension" %>
<%@ Import Namespace="Beeant.Domain.Entities.Account" %>
    ...
    <ContentTemplate>
       ...
       <Columns>
        ...
        <asp:TemplateField ItemStyle-CssClass="center ckbox">
            ...
            <ItemTemplate>
               <input value='<%#Eval("Id") %>' id="ckSelect" runat="server" type="checkbox" SubCheckName="selectall" ComfirmValidate="Remove,BindAccountNumber"  />
           </ItemTemplate>
        </asp:TemplateField>
         <asp:TemplateField HeaderText="编辑" ItemStyle-CssClass="center operate">
            ...
            <ItemTemplate>
                <a href='/Site/Company/Save.aspx?siteid=<%#Eval("Id") %>' target="_blank" name="Company">公司信息</a>
            </ItemTemplate>
        </asp:TemplateField>
        <asp:TemplateField HeaderText="编号"  ItemStyle-CssClass="center">
         ...
         <asp:TemplateField HeaderText="是否绑定"  ItemStyle-CssClass="left">
            <ItemTemplate>
               <%#GetBindName(Eval("Account").Convert<AccountEntity>())%>
            </ItemTemplate>
        </asp:TemplateField>
        ...
        <asp:TemplateField HeaderText="到期时间"  ItemStyle-CssClass="left">
            <ItemTemplate>
                <%#Eval("ExpireDate","{0:yyyy-MM-dd HH:mm}")%>
                  ...
                  <asp:TemplateField HeaderText="是否显示作者"  ItemStyle-CssClass="left">
            <ItemTemplate>
                <%#Eval("IsShowAuthorName")%>
            </ItemTemplate>
        </asp:TemplateField>
                 <asp:TemplateField HeaderText="是否启用密保"  ItemStyle-CssClass="left">
            <ItemTemplate>
                <%#Eval("IsPasswordName")%>
            </ItemTemplate>
        </asp:TemplateField>
     ...
     <uc1:Pager ID="Pager1" runat="server" PageSize="10"   SelectExp="Id,Setting,Account.AccountNumbers.Select(Name)" OrderByExp="Id desc"  />
