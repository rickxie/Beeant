<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="Add.aspx.cs" Inherits="Beeant.Presentation.Admin.Finance.Finance.Invoiceout.Add" MasterPageFile="~/Main.Master" ValidateRequest="false" %>
    ...
    <table class="tb">
        ...
        <tr>
         ...
         <tr>
           ...
           <tr>
               <td colspan="4" >
                   <a href="javascript:void(0);" id="hfAccountId" Note="note" NoteUrl='SelectOrder.aspx?SerializeValueId=<%=hfOrders.ClientID %>&SerializeContainerId=<%=gvInvoice.ClientID %>&accountId=<%=cbAccount.InputHidden.Value %>'>添加订单</a>
               </td>
        </tr>
    </table>
...
<div id="divOrder"   >
     ...
     <input  type="hidden" id="hfOrders" runat="server"  />
         <asp:GridView ID="gvInvoice" runat="server" AutoGenerateColumns="False" CssClass="tblist"  >
              <EmptyDataTemplate>
             <tr>
			<th scope="col" >订单编号</th><th scope="col" >状态</th><th >级别</th><th >所属人</th><th >提交人</th><th >提交时间</th><th >应收金额</th><th >已开金额</th><th >本次开票</th><th >备注</th>
		</tr>
            </EmptyDataTemplate>
       <Columns>
        ...
        <asp:TemplateField HeaderText="本次开票"  ItemStyle-CssClass="left">
            <ItemTemplate>
                <input value='<%# Convert.ToDouble(Eval("Amount").ToString())-Convert.ToDouble(Eval("InvoiceAmount").ToString()) %>' id="txtAmount" runat="server" type="text" class="input" />                
            </ItemTemplate>
        </asp:TemplateField> 
    ...
    <script type="text/javascript" src="/Scripts/Winner/Note/Winner.Note.js"></script>
     ...
     var registerFunc = function() {
         $(document).ready(function() {
             window.Serial = new Serializator({ Serializes: [{ Id: '<%=gvInvoice.ClientID %>', Html: $("#divTemplate").html(), ValueId: "<%=hfOrders.ClientID %>" }] });
             ...
             <%=cbAccount.ClientID %>.Select = function() {
                 ...
                 $("#hfAccountId").attr("NoteUrl", "SelectOrder.aspx?SerializeValueId=<%=hfOrders.ClientID %>&SerializeContainerId=<%=gvInvoice.ClientID %>&accountid="+accountid);
             };
         });
     }
