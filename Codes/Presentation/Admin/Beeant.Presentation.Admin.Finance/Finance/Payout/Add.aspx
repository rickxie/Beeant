<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="Add.aspx.cs" Inherits="Beeant.Presentation.Admin.Finance.Finance.Payout.Add" MasterPageFile="~/Main.Master" ValidateRequest="false" %>
...
<asp:Content ID="Content2" ContentPlaceHolderID="Body" runat="server">
 ...
 <div class="edit">
    ...
    <table class="tb">
       <tr>
            ...
            <td class="text"><asp:DropDownList ID="ddlStatus" runat="server" BindName="Status" SaveName="Status" ValidateName="Status" ></asp:DropDownList></td>
        ...
        <tr>
         ...
         <tr>
          ...
          <tr>
               <td colspan="4" >
                   <a href="javascript:void(0);" id="hfAccountId" Note="note" NoteUrl='SelectPurchase.aspx?SerializeValueId=<%=hfPurchases.ClientID %>&SerializeContainerId=<%=gvPaid.ClientID %>&accountId=<%=cbAccount.InputHidden.Value %>'>添加采购单</a>
               </td>
        </tr>
    </table>
 ...
 <div id="divPurchase" >
     <input  type="hidden" id="hfPurchases" runat="server"  />
         ...
         <asp:GridView ID="gvPaid" runat="server" AutoGenerateColumns="False" CssClass="tblist"  >
             <EmptyDataTemplate>
             <tr>
			<th scope="col" >采购单编号</th><th scope="col" >状态</th><th >级别</th><th >所属人</th><th >提交人</th><th >提交时间</th><th >应付金额</th><th >实付金额</th><th >本次开票</th><th >备注</th>
		</tr>
            </EmptyDataTemplate>
       <Columns>
        ...
        <asp:TemplateField HeaderText="应付金额"  ItemStyle-CssClass="left">
            <ItemTemplate>
                <%#Eval("TotalAmount")%>
            </ItemTemplate>
        </asp:TemplateField>  
    ...
    <script type="text/javascript" src="/Scripts/Winner/Note/Winner.Note.js"></script>
     ...
     var registerFunc = function() {
         $(document).ready(function() {
             window.Serial = new Serializator({ Serializes: [{ Id: '<%=gvPaid.ClientID %>', Html: $("#divTemplate").html(), ValueId: "<%=hfPurchases.ClientID %>" }] });
             window.Serial.Initialize();
             window.Serial.ResetFunction = function() {
                 var ctrls = $("#<%=gvPaid.ClientID %>").find("input[name='amount']");
                 var amount = 0;
             ...
             setTimeout(function() {
                 $("#<%=gvPaid.ClientID %>").css("table-layout", "auto");
             }, 1000);
             ...
             <%=cbAccount.ClientID %>.Select = function() {
                 ...
                 var myDate = new Date();
                 $("#<%=txtName.ClientID %>").val($("#<%=cbAccount.InputText.ClientID %>").val() + "/" + myDate.getFullYear()+"-"+myDate.getMonth()+"-"+myDate.getDate() + "/付款单");
                 $("#hfAccountId").attr("NoteUrl", "SelectPurchase.aspx?SerializeValueId=<%=hfPurchases.ClientID %>&SerializeContainerId=<%=gvPaid.ClientID %>&accountid="+accountid);
             };
         });
     }
