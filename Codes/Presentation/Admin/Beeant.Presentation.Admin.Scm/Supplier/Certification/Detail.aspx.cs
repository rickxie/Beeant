...
{
    public partial class Detail : DetailPageBase<CertificationEntity>
    {
        protected override CertificationEntity GetEntity()
        {
            ...
            query.Query<CertificationEntity>().Where(it => it.Id == RequestId)
                .Select(it => new object[] { it, it.Supplier.Name });
            return Ioc.Resolve<IApplicationService, CertificationEntity>().GetEntities<CertificationEntity>(query).FirstOrDefault();
        }
