...
//		calendar.hide();
        ...
        var nowDate = new Date(calendar.date.getFullYear(), calendar.date.getMonth(), this.innerHTML).format(calendar.date2StringPattern);
	    if (calendar.limitDate) {
	        var timePar = calendar.limitDate.split(' ');
	        var timeDate = timePar[0].split('-');
	        calendar.limitDate = timeDate[1] + '/' + timeDate[2] + '/' + timeDate[0];
	        var compare1 = new Date(calendar.limitDate).format(calendar.date2StringPattern);
	        if (compare1 > nowDate) {
	            calendar.dateControl.value = compare1;
	        } else {
	            calendar.dateControl.value = nowDate;
	        }
	    } else {
	        calendar.dateControl.value = nowDate;
	    }
		...
		if (dateArray[i]){
			tds[i].onclick = function () {
				if (calendar.dateControl) {
					var nowDate =  new Date(calendar.date.getFullYear(),
...
Calendar.prototype.show = function (dateControl, popuControl, limitDate) {
    ...
    if(limitDate!=null) this.limitDate = limitDate;
...
document.writeln('<div id="__calendarPanel" style="position:absolute;display:none;z-index:9999;background-color:#FFFFFF;border:1px solid #666666;width:200px;height:220px;">');
...
__ci.document.writeln('td {font-size:12px;text-align:center;padding:0;height:22px}');
