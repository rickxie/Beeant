...
using Beeant.Presentation.Website.Password.Models.Home;
using Winner.Filter;
using Winner.Persistence;
using Winner.Persistence.Linq;

namespace Beeant.Presentation.Website.Password.Controllers.Home
{
        ...
        protected virtual void HidePartEntity(PasswordModel model, AccountEntity account)
        {
            if (!string.IsNullOrEmpty(account.Mobile) && account.IsActiveMobile)
            {
                ...
                if (account.Mobile.Length == 11)
                {
                    ...
                    model.AccountWebsite = builder.ToString();
                }
            }
