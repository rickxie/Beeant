...
namespace Beeant.Presentation.Website.Editor.Controllers.Editor
{
    ...
    public class ImageController : BaseController
    {
        ...
        /// <returns></returns>
        protected virtual ImageEntity GetSaveEntity(string path, long folderId)
        {
        ...
        /// <param name="folderId"></param>
        /// <returns></returns>
        protected virtual ImageEntity Save(string path, long folderId)
        {
            var info = GetSaveEntity(path, folderId);
            if (info == null) return null;
            Ioc.Resolve<IApplicationService, ImageEntity>().Save(info);
            return info;
        }
        /// <summary>
        /// 得到错误信息
        ...
        /// 保存
        /// </summary>
        [DataFilter(EntityType = typeof(ImageEntity))]
        public virtual void Move(string id)
        {
            var info = new ImageEntity { Id = id.Convert<long>(), Folder = new FolderEntity { Id = Request["folderid"].Convert<long>() }, SaveType = SaveType.Modify };
        ...
        public virtual ActionResult Remove(string[] id)
        {
            ...
            if (id != null)
            {
                var infos = new List<ImageEntity>();
                foreach (var i in id)
                {
                    var info = new ImageEntity
                    {
