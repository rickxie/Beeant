$(document).ready(function () {
    ...
    function removeCustomerType(sender) {//删除
        if (!confirm("您确定要删除吗"))
            return ;
    ...
    function upCustomerType(sender) {
        if (!window.checkSaveTag(sender)) {
            return;
        }
        var id = $(sender).attr("DataId");
    ...
    function downCustomerType(sender) {
        if (!window.checkSaveTag(sender)) {
            return;
        }
        var id = $(sender).attr("DataId");
